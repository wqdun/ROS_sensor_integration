...
void MifReader::gpsCallback(const sc_server_daemon::monitorMsg::ConstPtr& pGpsMsg) {
    // lat: 1 degree is about 100000 m
    const double lat = pGpsMsg->lat_lon_hei.x;
    // lon: 1 degree is about 100000 m
    const double lng = pGpsMsg->lat_lon_hei.y;
    ...
    mCurrentWGS.z = pGpsMsg->lat_lon_hei.z;
