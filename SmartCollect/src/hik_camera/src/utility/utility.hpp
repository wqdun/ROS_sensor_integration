...
class Utility
{
  public:
    ...
    {
        Eigen::Vector3d n = R.block<3, 1>(0, 0);
        Eigen::Vector3d o = R.block<3, 1>(0, 1);
        Eigen::Vector3d a = R.block<3, 1>(0, 2);

        Eigen::Vector3d rpy(3);
        double y = atan2(o[0], o[1]);
        double p = atan2(-o[2], o[0] * sin(y) + o[1] * cos(y));
        double r = atan2(sin(y) * a[1] - cos(y) * a[0], cos(y) * n[0] - sin(y) * n[1]);
        rpy(0) = -r;
        rpy(1) = -p;
        rpy(2) = y;
    ...
    static cv::Mat toCvMatd(const Eigen::Matrix3d &m);
