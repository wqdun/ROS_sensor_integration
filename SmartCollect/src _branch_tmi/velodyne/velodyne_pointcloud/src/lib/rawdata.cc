...
namespace velodyne_rawdata
{
  ...
  void RawData::unpack_vlp16(const velodyne_msgs::VelodynePacket &pkt,
                             VPointCloud &pc, VPointCloud &pcImuCoord, std::vector<sensor_msgs::PointCloud2> &pcVec)
  {
    float azimuth;
    float azimuth_diff;
        ...
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          if ((azimuth_corrected >= config_.min_angle
            ...
            if (pointInRange(distance)) {
              ...
              VPoint point;
              VPoint imuPoint;
              point.ring = imuPoint.ring = corrections.laser_ring;
              point.x = x;
              point.y = y;
              point.z = z;
              point.intensity = imuPoint.intensity = intensity;
    ...
    pcl::toROSMsg(pcImuCoord, outPc);
    pcVec.push_back(outPc);
  }
