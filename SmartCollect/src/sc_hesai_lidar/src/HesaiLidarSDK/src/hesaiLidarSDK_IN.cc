...
void HesaiLidarSDK_internal::WriteLidarFile(const PandarPacket &pandarPacket) {
    FILE *pOutFile;
    if(!(pOutFile = fopen("/tmp/hesai.pcap", "ab") ) ) {
        LOG(ERROR) << "Create file failed, errno:" << errno;
        exit(1);
    }
    DLOG(INFO) << "Create file successfully.";

    fwrite(&pandarPacket.data[0], pandarPacket.size, 1, pOutFile);
    fclose(pOutFile);
}
...
void HesaiLidarSDK_internal::lidarRecvTask()
{
	while (continueLidarRecvThread)
	{
        ...
        // TODO: WriteLidarFile
		pushLiDARData(pkt);
		LOG(INFO) << "pkt.stamp: " << std::fixed << pkt.stamp;
		LOG(INFO) << "pkt.size: " << std::fixed << pkt.size;
        WriteLidarFile(pkt);
