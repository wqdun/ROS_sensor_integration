...
namespace velodyne_rawdata
{
  ...
  {
    mAngleLidar2Imu = {
      public_tools::PublicTools::deg2rad(30), 0, 0,
    };
    mOffsetLidar2Imu = {
      0, 0, 0.3187,
    };
  ...
  void RawData::unpack_vlp16(const velodyne_msgs::VelodynePacket &pkt,
                             VPointCloud &pc, std::vector<VPointCloud> &pcVec)
  {
    float azimuth;
    float azimuth_diff;
    ...
    VPointCloud pcImuCoord;
    pcImuCoord.points.clear();
    // LIDAR day second: UTC time, should +18 to GPS time
    pcImuCoord.daytime = pkt.stamp.toSec() + 18;
    ROS_DEBUG_STREAM("Day Time: " << std::fixed << pcImuCoord.daytime);
        ...
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          if ((azimuth_corrected >= config_.min_angle
            ...
            if (pointInRange(distance)) {
              ...
              imuPoint.intensity = intensity;

              pcImuCoord.vPoints.push_back(imuPoint);
            }
          }
        }
      }
    }
    pcVec.push_back(pcImuCoord);
  }
