...
void dataFixed::mkImageTraceData(double imageTime,std::string &projectName,std::vector <ontimeDataFormat> &imuData,std::string &reNewPicture,imageTraceDataFormat &oneImageTraceData)
{
    ...
    double timeInvalRecord=10.0;
    unsigned long imuDataSubMark=0;
    unsigned long timeMatchedMark=0;
    for(unsigned long k = 0;k < imuData.size();k++)
    {
        double timeInval=fabs(imageTime-imuData[k].GPSWeekTime);
...
void dataFixed::readOntimeTraceData(std::string &projectPath,std::vector<ontimeDataFormat> &traceFileData){
    ...
    {
        std::cout << "Creat IMU real Time Data Pipe Failed" << std::endl;
        exit(1);
    }
    while(1)
    {
    ...
    if(0 != pclose( fpin ) )
    {
        std::cout << "close IMU Pipe Failed" <<std::endl;
        exit(1);
    }
    if(filesPath.size() != 1)
    {
        std::cout << "Load Number of IMU real time Data is not 1" << std::endl;
        exit(1);
    }
    ...
    for(unsigned long i=0; i < editePictureNameMark.size() ; i++)
    {
        ...
        else
        {
            ...
            if(NULL == ( deleImageFpin = popen(deleImagecmd.c_str(),"w" ) ) )
                std::cout << "delete Picture falied"<< std::endl;
...
void dataFixed::mkLidarTraceFile(std::string &projectPath,std::vector <ontimeDataFormat> &imuData){
    ...
    for(int i = 0; i < totalLidarFilePath.size();i++)
    {
        ...
        {
            std::cout << "read the Lidar :" << totalLidarFilePath[i] << "Falied" << std::endl;
            continue;
        }
        ...
        while(tmpNum <= pktNum)
        {
            ...
            std::cout<<"Lidarpoint timeStamp is:"<<oneLidarPointTime<<std::endl;
            ...
            {
                for(unsigned long k=0;k <lidarPointGPSTimeMark.size();k++)
                {
