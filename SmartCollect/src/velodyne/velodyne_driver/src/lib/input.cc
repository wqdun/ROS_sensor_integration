...
namespace velodyne_driver
{
    ...
    Input(private_nh, port)
  {
    ...
    mSockfdPosition = socket(PF_INET, SOCK_DGRAM, 0);
    if (sockfd_ == -1 || -1 == mSockfdPosition)
      {
    ...
    sockaddr_in my_addr_position;
    memset(&my_addr_position, 0, sizeof(my_addr_position));
    my_addr_position.sin_family = AF_INET;
    my_addr_position.sin_port = htons(POSITION_PORT_NUMBER);
    my_addr_position.sin_addr.s_addr = INADDR_ANY;
    ...
    if (bind(mSockfdPosition, (sockaddr *)&my_addr_position, sizeof(sockaddr)) == -1)
      {
        perror("mSockfdPosition bind");
        return;
      }
    ...
    if (fcntl(mSockfdPosition, F_SETFL, O_NONBLOCK|FASYNC) < 0)
      {
        perror("mSockfdPosition non-block");
        return;
      }
    ...
    while (true)
      {
        ...
        do
          {
            ...
            if ((fds[1].revents & POLLERR)
                ...
                || (fds[1].revents & POLLNVAL)) // device error?
              {
                ROS_ERROR("mSockfdPosition poll() reports Velodyne error");
                return 1;
              }
          }
        ...
        ssize_t nbytes = -1;
        // get packet from 2368 port: long frame
        if(!_isPositionPkt) {
          nbytes = recvfrom(sockfd_, &pkt->data[0],
                                    packet_size,  0,
        ...
        // get packet from 8308 port: short frame/position packet
        else {
          nbytes = recvfrom(mSockfdPosition, _positionPkt,
                                    POSITION_PACKET_SIZE,  0,
                                    (sockaddr*) &sender_address,
                                    &sender_address_len);
          if((size_t) nbytes == POSITION_PACKET_SIZE) {
            // read successful,
            // if packet is not from the lidar scanner we selected by IP,
            // continue otherwise we are done
            if(devip_str_ != ""
  ...
  /** @brief Get one velodyne packet. */
  int InputPCAP::getPacket(velodyne_msgs::VelodynePacket *pkt, const double time_offset, bool &_isPositionPkt, char *_positionPkt)
  {
