...
FeatureManager::FeatureManager(Matrix3d _Rs[], Matrix3d ric)
: Rs(_Rs)
{
 ...
 */
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td,int &parallax_num,bool solver_flag,int &solved_num,int &feature_num)
{
   ...
   //printf("size of img = %lu\n",image_msg.size());
    for (auto &id_pts : image)
    {
        ...
        else if (it->feature_id == feature_id)
        {
            ...
            if(it->used_num > 2)
            {
             ...
             if(solver_flag)
             {
                 if(it->solve_flag == 1 && it->used_num >= 2 && it->start_frame < WINDOW_SIZE - 2)
                 {
                     solved_num++;
                 }
    ...
    if (parallax_num == 0 || feature_num < 20)
    {
        return true;
    }
    else
