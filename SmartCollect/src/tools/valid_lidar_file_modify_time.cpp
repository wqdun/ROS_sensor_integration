...
int main(int argc, char **argv) {
    ...
    const std::string outFile(inFile + "_calibrated");
    ...
    do {
        ...
        double timeCalibrated = time2 + 7200;
        if (time2 < 30000) {
            timeCalibrated = time2 + 10800;
        }
        (void)fwrite(&timeCalibrated, sizeof(timeCalibrated), 1, pOutFile);
        (void)fwrite(&(pktInfo.data[0]), 1206, 1, pOutFile);
