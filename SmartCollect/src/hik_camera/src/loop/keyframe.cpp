                 ...
                 vector<cv::Point2f> &measurements_old_norm)
{
    if (measurements_old.size() >= 8)
    {
        ...
        for (unsigned int i = 0; i < measurements_old.size(); i++)
        {
            ...
            cv::Point2f norm_pt;
            Eigen::Vector2d a(measurements_old[i].x,measurements_old[i].y);
            Eigen::Vector3d b;
            g_camera->liftProjective(a,b);
            norm_pt.x = b.x() / b.z();
            norm_pt.y = b.y() / b.z();
									 ...
									 std::vector<cv::Point2f> &measurements_predict)
{
	...
	for(int i = 0; i < (int)point_clouds.size(); i++)
	{
    	...
    	pts_predict <<  pts_camera_j.x()/pts_camera_j.z(),
    					...
    					1.0;
    	            ///////////delete after debug!/////////////////
                    double FOCUS_LENGTH_X=520;
                    double FOCUS_LENGTH_Y=516;
                    double PX=320;
                    double PY=240;
                    ////////////////////////////////////////////////
        Vector2d point_uv;
...
{
	Matrix3d R_w_c = R_w_i * qic;   //R_w^c  from w to c   qic == q_i^c
                                 ...
                                 vector<cv::KeyPoint> &keys, vector<BRIEF::bitset> &descriptors) const
{
    ...
    //vector<cv::KeyPoint> seekeys=keys;
